...
if (fs.existsSync(configPath)) {
  ...
  let seenUrl = false, seenBase = false;
  const newLines = [];
  for (let line of lines) {
    if (/^\s*url\s*:/i.test(line)) {
      if (!seenUrl) { seenUrl = true; newLines.push(line); }
      else continue;  // skip duplicate url
    } else if (/^\s*baseurl\s*:/i.test(line)) {
      if (!seenBase) { seenBase = true; newLines.push(line); }
...
const commonPatterns = ['node_modules/', '_site/', '.sass-cache/', '.jekyll-cache/', '.bundle/', 'Gemfile.lock'];
...
if (fs.existsSync(consolidatePath)) {
  ...
  const cleanedConsolidate = consolidateText.replace(/const _FILE_PATTERNS\s*=\s*\[[\s\S]*?\];\r?\n/, '');
...
if (fs.existsSync(logosPath)) {
  ...
  if (!logosText.includes("import sharp from 'sharp'")) {
    logosText = logosText.replace('import { exec } from \'child_process\';', 'import { exec } from \'child_process\';\nimport sharp from \'sharp\';');
  }
  // Remove the tool-checking and external conversion functions
  logosText = logosText.replace(/async function checkTools\([\s\S]*?process\.exit\(1\);\s*}\s*/m, '');
  logosText = logosText.replace(/const tool = await checkTools\(\);\s*[\s\S]*?console\.log\(`Using: .*?\n/, '');
  logosText = logosText.replace(/async function convertWithImageMagick[\s\S]*?async function convertSvgToPng[\s\S]*?\}\s*}/, '');
  // Replace conversion loop to use sharp
  logosText = logosText.replace(/try\s*{\s*await convertSvgToPng\(.*?}\s*catch\s*\(error\)\s*{\s*console\.error\([\s\S]*?failed\+\+;/m,
    `try {
...
if (fs.existsSync(unusedCSSPath)) {
  ...
  const improvedRegexText = unusedText.replace(/matchAll\(\s*\/class="([^"]*)"\/g\)/, 'matchAll(/class="([^"]*)"|class=\'([^\']*)\'/g)');
...
const legacyPatterns = [/^TILLERSTEAD.*\.txt$/i, /^FINAL_REPORT\.txt$/i, /PHASE\d+_COMPLETE.*\.txt$/i, /REPORT\.txt$/i, /_COMPLETE\.txt$/i];
  ...
  const walkDir = (dir) => {
    fs.readdirSync(dir).forEach(name => {
      ...
      if (fs.statSync(fullPath).isDirectory()) {
        ...
        const relPath = path.relative(sassDir, fullPath).replace(/\\/g, '/').replace(/\.scss$/, '');
...
if (fs.existsSync(assetsImgDir)) {
  ...
  const dirsToScan = ['_config.yml', '_data', '_includes', '_layouts', 'pages', '_posts', path.join('assets', 'css')];
